...
int gpu_rnnt_delay(torch::Tensor acts,
                  torch::Tensor labels,
                  torch::Tensor input_lengths,
                  torch::Tensor label_lengths,
                  torch::Tensor delay_values,
                  torch::Tensor costs,
                  torch::Tensor grads,
                  float delay_scale,
                  float smooth,
    ...
    switch (acts.type().scalarType()) {
      case torch::ScalarType::Float:
        {
        size_t gpu_size_bytes;
        get_delay_workspace_size(maxT, maxU, minibatch_size,
                           true, &gpu_size_bytes);
        ...
        compute_rnnt_delay_loss(acts.data<float>(), grads.data<float>(),
                         labels.data<int>(), label_lengths.data<int>(),
                         input_lengths.data<int>(), delay_values.data<float>(), alphabet_size,
                         minibatch_size, costs.data<float>(),
                         gpu_workspace,delay_scale, smooth, options);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("gpu_rnnt_delay", &gpu_rnnt_delay, "Transducer with delay");
#endif
}
