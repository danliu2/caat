olog_dir=./log_inferwaitk_mstep_run2
mkdir -p $olog_dir
...
do
	...
	for step in ${steps[@]};do
        name=${model}_${k}_step${step}_beam5_forcast2
        ...
        cmd="simuleval --agent ./rain/simul/speech_waitk.py \
            ...
            --wait-blocks $k --step-read-blocks $step --step-generate $step \
            --output $outdir  --port $port --timeout 100 --beam 5 --step-forecast 2 --stop-early"
        func_sub $name "$cmd"
        port=$[port+1]


        name=${model}_${k}_step${step}_beam5_naiive
        outdir=$olog_dir/$name
        cmd="simuleval --agent ./rain/simul/speech_waitk.py \
            --task-type st --train-dir exp_data/must_filtered2 \
            --source-lang en --target-lang de \
            --source $wavfile --target $tgtfile \
            --data-type speech --task s2s --model-path $modelfile \
            --wait-blocks $k --step-read-blocks $step --step-generate $step  --step-forecast 0 \
