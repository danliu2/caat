...
work_dir=/work/superbrain/danliu/snmt_7/
mkdir -p $work_dir
olog_dir=./log
mkdir -p $olog_dir
...
name=mt_shallowmodel
cmd="fairseq-train $mt_dir --source-lang $SRC --target-lang $TGT \
    --max-epoch 100 \
    --user-dir rain \
    --ddp-backend=no_c10d \
    --task s2s --task-type mt \
    --bpe-dropout 0.1 \
    --arch mt_shallowmodel --dropout 0.3 --activation-dropout 0.1 \
...
cmd="fairseq-train $mt_dir --source-lang $SRC --target-lang $TGT \
    ...
    --user-dir rain \
    --ddp-backend=no_c10d \
    --task transducer --task-type mt \
    --bpe-dropout 0.1 \
    --arch mt_transducer --dropout 0.3 --activation-dropout 0.1 \
    --share-decoder-input-output-embed \
    --share-all-embeddings   \
    --optimizer adam --adam-betas '(0.9, 0.98)' \
    --lr 0.0005 --lr-scheduler inverse_sqrt  \
...
name=audio_shallowmodel
cmd="fairseq-train $st_dir --source-lang $SRC --target-lang $TGT \
    --user-dir rain \
    --max-epoch 80 \
    --ddp-backend=no_c10d \
    --task s2s --task-type st --bpe-dropout 0.1 \
    --arch audio_shallowmodel \
    ...
    --save-dir $work_dir/$name \
    --max-tokens 20000 \
    --update-freq 8 --max-sentences 64 --skip-invalid-size-inputs-valid-test \
    --log-interval 10 --save-interval 4 --log-format simple  --num-workers 2"
#func_sub $name "$cmd"

name=audio_transducer
cmd="fairseq-train $st_dir --source-lang $SRC --target-lang $TGT \
    --user-dir rain \
    --max-epoch 80 \
    --ddp-backend=no_c10d \
    --task transducer --task-type st --bpe-dropout 0.1 \
    --arch audio_transducer \
    --dropout 0.1 --activation-dropout 0.1 --attention-dropout 0.1 \
    --share-decoder-input-output-embed \
    --optimizer adam --adam-betas '(0.9, 0.98)' \
    --lr 0.0005 --lr-scheduler inverse_sqrt  \
